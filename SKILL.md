...
保持 skill 包无状态。`SKILL.md`、`references/` 和 `scripts/` 只放静态能力；每次批量任务的输入、taxonomy、RAG、状态库和结果都放到用户指定的 `workdir`。不要把用户数据、API key、运行状态写入 skill 目录。

## 输入分流

- 小于 10 条记录：使用 Inline Mode。不创建 `workdir`，直接按 `references/topk-prompt.md` 和 `references/final-prompt.md` 在当前响应中返回结果。
...
1. 确认用户提供 `classification_options`。没有分类选项时停止并要求提供。
2. 将每条记录标准化为字符串。表格记录必须保留字段标签，例如 `问题概述：...`、`问题明细：...`、`问题根因：...`、`解决方案：...`。
3. 每条记录只检索一次 RAG，形成 `rag_pool`。TopK 使用前 3 条；Final 使用候选池内优先的前 5 条。
4. TopK 阶段输入记录数组、分类选项和 RAG，输出多个候选 `level_1/level_2/confidence`。
...
6. Runner 根据 TopK 置信度动态决定候选池大小：
   ...
   - 否则最多传 6 个候选；
   - TopK 返回更少候选时不补齐。
7. Final 阶段输入单条或批量 item：问题记录、候选池、RAG 结果。Final 只能从候选池中选择最终一级/二级。
...
- TopK 输入：`records` 字符串数组、`classification_options`、每条记录的 `rag_results`。
...
- `references/topk-prompt.md`：TopK prompt 输入输出契约。
- `references/final-prompt.md`：Final prompt 输入输出契约。
